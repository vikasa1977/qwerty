...
//  console.log("Location", inspectedObject.part.location)
  $(this._div).show();
  ...
  var mainDiv = this._div;
    //  $(mainDiv).css({"top":"calc( 50% + "+inspectedObject.location.y+"px)", "left":"calc( 50% + "+inspectedObject.location.x+"px)"});
  $(mainDiv).css({
      top: "calc( 100% - 300px )",
      left: "calc( 100% - 200px)"
  });
  mainDiv.innerHTML = "<div class='collapsible-header'>PROPERTIES PANEL</div>";
  ...
  if (this.includesOwnProperties) {
  	var propertiesToDislplay = ["name", "attribute", "operator", "value", "leadApprover", "enableAutoApproval", "selfApproval", "poolType", "PoolValue"];
  	for (var k in data) {
  		if (propertiesToDislplay.indexOf(k) != -1 && angular.isDefined(data[k])) {
...
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue) {
 ...
 //var td2 = document.createElement("td");
  if (propertyName == "operator") {
  	...
  	selectOptions = ["Equals To", "Not Equals To", "Less Than", "Greater Than"];

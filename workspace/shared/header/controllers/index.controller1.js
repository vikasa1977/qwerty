(function (angular) {
	...
	angular
		...
		.controller('headerCtrl', ['$scope', '$rootScope', '$translate', '$sce', '$compile', 'APPCONSTANTS', '$state', 'notification', '$timeout', 'redirectState', 'requestClicked', headerCtrlFunc]);

	function headerCtrlFunc($scope, $rootScope, $translate, $sce, $compile, APPCONSTANTS, $state, notification, $timeout, redirectState, requestClicked) {
		$scope.showMenu = true;
		...
		$scope.gotoHomePage = function () {
			...
			if ($scope.showUserProfileOverlay) {
				$scope.showUserProfileOverlay = false;
				$timeout(function () {
					window.location.href = 'index_launcher1.html#/platform';
				}, 500);
			} else {
			    window.location.href = 'index_launcher1.html#/platform';
		...
		$scope.announcement = {
			...
			setType: function (item, index, dataSet) {
				if (this.data.type == "Smart") {
					this.urlState = true;
					this.urlRead = true;
					this.urlStateText = "https://gepmtstorage.blob.core.windows.net/smart2ux/workspace/index_launcher1.html#";
				}

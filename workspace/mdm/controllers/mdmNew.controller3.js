...
.service('MDMService', ['$http','$q','$window','$timeout', MDMServiceFn  ])
   .controller('mdmCreateNewCtrl2', ['$scope', '$rootScope', '$translate', 'RuleEngine', '$http', '$state', 'notification', '$timeout', '$filter', '$sce', '$interval', '$notification', 'storeService', '$window', 'MDMService',mdmCreateNewCtrl2Func])
   .controller('mdmNewItemDetailCtrl1', ['$scope', '$rootScope', '$translate', 'RuleEngine', '$http', '$state', 'notification', '$timeout', '$filter', '$sce', '$interval', '$notification', 'storeService', '$window', 'MDMService',mdmNewItemDetailCtrl1Func])
   // .controller('MDMBasicDetailsCtrl', ['$scope', '$rootScope', '$state', '$http', '$timeout', 'lookup', MDMBasicDetailsCtrlFunc])


function mdmCreateNewCtrl2Func($scope, $rootScope, $translate, RuleEngine, $http, $state, notification, $timeout, $filter, $sce, $interval, $notification, storeService, $window, MDMService) {
    
//     $scope.itemNumber ="43243253";
//     $scope.supplierName ="BW Roger";
//     $scope.price ="89.75";
   ...
   //$scope.selectedCurrencyData = { "code": "€", "name": "USD" };
   ...
   $scope.onChange = function(selectedCurrency) {
	...
	if($state.params.status === "readonly"){
		...
		$scope.docName = "REQ-2017114 - Volvo Lastvagnar AB - RECOMMENDATION";
		$scope.showStatus = false;
		var urlJson = 'mdm/models/createMDM2.json';
		$scope.boreGeo = '';
...
function mdmNewItemDetailCtrl1Func($scope, $rootScope, $translate, RuleEngine, $http, $state, notification, $timeout, $filter, $sce, $interval, $notification, storeService, $window, MDMService) {
	MDMService.getFormConfigData();
	$scope.status = $state.params.status;
	//$state.params.status = "Submit";
	//alert($scope.status);	
   ...
   $scope.onChange = function(selectedCurrency) {
	...
	if($state.params.status === "readonly"){
		...
		$scope.clearance = 'C3';
		$scope.noOfSeals = '2 Shield'
	} else if(!$state.viewScenario) {
		$scope.docName = "4,795 SQ FT Restaurant";
		$scope.showStatus = false;
		var urlJson = 'mdm/models/itemDetails1.json';
		$scope.boreGeo = '';
		$scope.InsideDiameter = '';
		$scope.outsideDiameter = '';
		$scope.width = '';
        ...
        $scope.docName = "Should Cost Analysis: 1234-Acrylonitrile-Scenario 1";
		$scope.showStatus = false;
		var urlJson = 'mdm/models/acrylonitrile_scenario1.json';
		$scope.boreGeo = '';
		$scope.InsideDiameter = '';
		$scope.outsideDiameter = '';
		$scope.width = '';
    ...
    $scope.sendForReview =function() {
        $state.go('expandedLandingList', { pagefor: 'manage', doctype: 'SHOULD COST ANALYSIS' }); 
    }
...
*/
   ...
   ****/
   ...
   $scope.tempCurrent = '',
    ...
    $timeout(function(){
        $('[label=Category], [label=Region]').find('input').css({'color':'#0177d6', 'cursor': 'pointer'});
        $('input[title="Robert Bosch LLC"]').css({'color':'#0177d6', 'cursor': 'pointer'});
        $('input[title="Hearthside Food Solutions"]').css({'color':'#0177d6', 'cursor': 'pointer'});
    },3000);
...
function MDMBasicDetailsCtrlFunc($scope, $rootScope, $state, $http, $timeout, lookup) {
        ...
        ////};
        $scope.typeSelectOwnedBy = [{ "name": "Company Name" }, { "name": "Partner Code" }, { "name": "D-U-N-S No." }, { "name": "US Federal Tax ID" }, { "name": "Social Security No." }, { "name": "VAT Reg. No." }, { "name": "GST/ QST No." }, { "name": "LEI" }, { "name": "SIC Code" }, { "name": "NASIC Code" }, { "name": "TIN No." }];
        ...
        $scope.dupSupData = {
            ...
            "fedralTaxDataRead": [{ "name": "Region 0", "check": true, "value": [{ "name": "Region child-0", "check": true, "value": [{ "name": "Region grand-child-0", "check": true }, { "name": "Region grand-child-1", "check": true }, { "name": "Region grand-child-2", "check": true }] }] }]
        };
        $scope.showInfoIcon = false;
        $scope.companyName = {
            "displaytext": "Company Name",
            "selectedoption": "",
            "options": [{ "name": "Quanta Inc", "check": false }, { "name": "kellogg's", "check": false }, { "name": "Evertek Inc", "check": false }, { "name": "GEP", "check": false }, { "name": "Rackspace Inc", "check": false }, { "name": "Globulent Travels", "check": false }, { "name": "Datamatic Ltd", "check": false }, { "name": "CDW", "check": false }, { "name": "Staples Inc", "check": false }, { "name": "IT Micro System Corporation", "check": false }, { "name": "Grainger Ltd", "check": false }, { "name": "Relix Inc", "check": false }, { "name": "Home Depot Co.", "check": false }]
        }
        $scope.status = {
            "displaytext": "Category",
            "selectedoption": { "name": "Choose Category", "isdisabled": false, "description": "This is Invited", "approved": true },
            "options": [
				{ "name": "Approved", "isdisabled": false, "description": "This supplier in disqualified and cannot be included for any future RFP, bidding and other activities. Limit this content to 2 lines", "approved": true },
				{ "name": "Disqualified", "isdisabled": false, "description": "This is disqualified", "approved": true }
				// ,
				// { "name": "Approval Pending", "isdisabled": false, "description": "This is approval pending", "approved": true },
				// { "name": "Identified", "isdisabled": false, "description": "This is identified", "approved": false },
        ...
        $scope.supplierIcardlink = function () {
            ...
            $state.go('supplier.profile', { pagefor: $scope.supplierIcard.supplierName });
        };
       ...
       { id: '3', name: 'INVOICE RECONCILIATION', count: '8', number: 'IR-2016.234829', url: 'invoice.html', isChecked: false },
        ...
        // region
        $scope.region = [{ "name": "Asia", "value": [{ "name": "India" }, { "name": "Japan" }, { "name": "Nepal" }] }];
        ...
        $scope.suppliermDataRead = [{ "name": "Carissa Madden", "check": true }, { "name": "Dotson Palmer", "check": true }, { "name": "Meyer Lloyd", "check": true }, { "name": "Flossie Ochoa", "check": true }, { "name": "Leah Moses", "check": true }];
        $scope.suppliermData = [{ "name": "Carissa Madden", "check": false }, { "name": "Dotson Palmer", "check": false }, { "name": "Meyer Lloyd", "check": false }, { "name": "Flossie Ochoa", "check": false }, { "name": "Leah Moses", "check": false }, { "name": "Ferguson Osborn", "check": false }, { "name": "Peck Patterson", "check": false }, { "name": "Gay Payne", "check": false }, { "name": "Katie Hebert", "check": false }, { "name": "Bryan Shannon", "check": false }, { "name": "Skinner Farmer", "check": false }, { "name": "Mckay Mcneil", "check": false }, { "name": "Lila Horne", "check": false }, { "name": "Ethel Powell", "check": false }, { "name": "Spears Lott", "check": false }, { "name": "Nannie Ryan", "check": false }, { "name": "Joy Ware", "check": false }, { "name": "Shaffer Mcfadden", "check": false }, { "name": "Audrey Pena", "check": false }, { "name": "Helga Macdonald", "check": false }];
        $scope.businessUnitData = [{ "name": "Business unit 0", "check": false, "value": [{ "name": "Business unit child-0", "check": false, "value": [{ "name": "Business unit grand-child-0", "check": false }, { "name": "Business unit grand-child-1", "check": false }, { "name": "Business unit grand-child-2", "check": false }] }, { "name": "Business unit child-1-0", "check": false, "value": [{ "name": "Business unit grand-child-1-0", "check": false }, { "name": "Business unit grand-child-1-1", "check": false }, { "name": "Business unit grand-child-1-2", "check": false }] }] }, { "name": "Business unit 1", "check": false, "value": [{ "name": "Business unit child-0", "check": false, "value": [{ "name": "Business unit grand-child-0", "check": false }, { "name": "Business unit grand-child-1", "check": false }, { "name": "Business unit grand-child-2", "check": false }] }, { "name": "Business unit child-1-0", "check": false, "value": [{ "name": "Business unit grand-child-1-0", "check": false }, { "name": "Business unit grand-child-1-1", "check": false }, { "name": "Business unit grand-child-1-2", "check": false }] }] }, { "name": "Business unit 2", "check": false, "value": [{ "name": "Business unit child-0", "check": false, "value": [{ "name": "Business unit grand-child-0", "check": false }, { "name": "Business unit grand-child-1", "check": false }, { "name": "Business unit grand-child-2", "check": false }] }] }];
        ...
        if ($scope.Pagefor.pagefor) {
            ...
            if ($scope.Pagefor.type) {
                $scope.status.selectedoption = { description: "This is approval pending", name: "Approval Pending" };
                $scope.selectTypeOption = {
                    "name": "status",
                    "selectiontext": " Approval Pending"
                };
            } else {
                $scope.status.selectedoption = { description: "This is invited", name: "Invited" };
                $scope.selectTypeOption = {
                    "name": "status",
                    "selectiontext": "Invited"
                };
            ...
            if ($scope.Pagefor.pagefor === "Quanta" || $scope.Pagefor.pagefor === "Fastenal V-Tech" || $scope.Pagefor.pagefor === "kellogg's" || $scope.Pagefor.pagefor === "Home Depot") {
                $scope.status.selectedoption = { "name": "Approved", "isdisabled": true, "description": "This supplier in disqualified and cannot be included for any future RFP, bidding and other activities. Limit this content to 2 lines" };
                $scope.selectedOwnedByoption = { name: "Partner Code" };
                $scope.companyName.selectedoption = { name: "Globulent Travels", check: false, value: "Globulent Travels" };
                $scope.selectTypeOption = {
                    "name": "status",

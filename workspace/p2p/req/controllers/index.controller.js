angular.module('SMART2')
    .controller('p2pReqCtrl', ['$scope', '$rootScope', 'RuleEngine', '$http', 'notification', p2pReqCtrlFunc])
	.controller('shipToCtrl', ['$scope', '$rootScope', shipToCtrlFunc])
    .controller('billToCtrl', ['$scope', '$rootScope', billToCtrlFunc]);

function p2pReqCtrlFunc($scope, $rootScope, RuleEngine, $http, notification) {
        	...
        	$scope.deleteReq = function () {
        	    notification.notify(deleteReqWarning, function (responce) {
        	        var result = responce.result;
        	        if (result == 'yes') {
        	            history.go(-1);
        	            $rootScope.$broadcast('itemDelet', { itemFrom: 'req' });
